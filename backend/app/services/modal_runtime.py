...
def _create_web_app() -> FastAPI:
    ...
    async def lifespan(web_app: FastAPI):
        ...
        print(f"[modal_runtime] starting vLLM: {' '.join(command)}", flush=True)
    ...
    async def health():
        ...
        if proc.poll() is not None:
            return JSONResponse(
                {"status": "error", "message": "vLLM process exited"},
                status_code=503,
            )
        try:
        ...
        if response.status_code != 200:
            return JSONResponse(
                {"status": "error", "message": f"upstream health {response.status_code}"},
                status_code=503,
            )
        return {"status": "ok"}
    ...
    async def models():
        try:
            response = await web_app.state.client.get(
                ...
                timeout=httpx.Timeout(connect=5.0, read=5.0, write=5.0, pool=None),
            )
